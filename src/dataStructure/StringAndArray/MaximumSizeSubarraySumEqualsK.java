...
http://www.programcreek.com/2014/10/leetcode-maximum-size-subarray-sum-equals-k-java/
  ...
  类似题目: SubarraySum.java, SubarraySumEqualsK.java
 */
public class MaximumSizeSubarraySumEqualsK {
	...
	static class Solution1{
		public static int maxSubArrayLen(int[] nums, int k) {
			...
			int maxLen =0 , sum =0;
			// 下面这一步很关键, 前面0个数相加和为sum=0, index 为 -1  (用下面同样的表达式即 i+1 =0)
			prefixSumMap.put(sum, -1);
			for(int i =0; i<nums.length;i++){
				sum += nums[i];
                //如果有相同prefixSum出现，我们只保存第一次出现的index
				//这样才能保证 maximum length of subarray sum.
	...
	public static void main(String[] args) {
		...
		int[] a3 = {-3,5,3};
		System.out.println(Solution1.maxSubArrayLen(a1, 13));
		System.out.println(s2.maxSubarrayLen(a1, 13));
		
		/*int[] a2 = {-2,6,8,-3,7,-1,12, -6, 0,6,9 };
		System.out.println(Solution1.maxSubArrayLen(a2, 9));
