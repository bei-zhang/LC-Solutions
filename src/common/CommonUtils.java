...
/**
 * copy from 
 * https://github.com/gaylemcd/ctci/blob/master/java/CtCILibrary/CtCILibrary/AssortedMethods.java
 * 
 * @author Blair
 *
 */
...
/*	public static TreeNode randomBST(int N, int min, int max) {
	...
	public static String getLongTextBlob() {
		String book = "As they rounded a bend in the path that ran beside the river, Lara recognized the silhouette of a fig tree atop a nearby hill. The weather was hot and the days were long. The fig tree was in full leaf, but not yet bearing fruit. "
				+ "Soon Lara spotted other landmarks梐n outcropping of limestone beside the path that had a silhouette like a man抯 face, a marshy spot beside the river where the waterfowl were easily startled, a tall tree that looked like a man with his arms upraised. They were drawing near to the place where there was an island in the river. The island was a good spot to make camp. They would sleep on the island tonight."
				+ "Lara had been back and forth along the river path many times in her short life. Her people had not created the path梚t had always been there, like the river梑ut their deerskin-shod feet and the wooden wheels of their handcarts kept the path well worn. Lara抯 people were salt traders, and their livelihood took them on a continual journey. ";
